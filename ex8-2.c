...
zipnode_t *search_node(zipnode_t *head, char *fulladdr) {
  int key = hash(fulladdr);
  zipnode_t *node = h_table[key];
...
int read_from_csv(zipnode_t **head, FILE *infp) {
  ...
  while (fgets(instr, BUF_SIZE, infp) != NULL) {
    ...
    if (add_node(head, zip, fulladdr) == 1) {
      return -1;
    }

    int key = hash(fulladdr);
    h_table[key] = (*head)->next;
    ++n;
  }
  return n;
}
...
int main() {
  ...
  int data_n = read_from_csv(&head, infp);
  if (data_n == -1) {
    fclose(infp);
    free_list(head);
    return 1;
  ...
  while (1) {
    ...
    result = search_node(head, fulladdr);
    if (result == NULL) {
      printf("no data\n");
    } else {
      printf("%d\n", result->zip);
