...
int main(void) {
  ...
  a = ((unsigned int)bytes[0]) |
      ((unsigned int)bytes[1] << 8) |
      ((unsigned int)bytes[2] << 16);
  b = ((unsigned int)bytes[3]) |
      ((unsigned int)bytes[4] << 8) |
      ((unsigned int)bytes[5] << 16);

  unsigned long long product = (unsigned long long)a * b;
  unsigned char result[6];
  result[0] = product & 0xff;
  result[1] = (product >> 8) & 0xff;
  result[2] = (product >> 16) & 0xff;
  result[3] = (product >> 24) & 0xff;
  result[4] = (product >> 32) & 0xff;
  result[5] = (product >> 40) & 0xff;
